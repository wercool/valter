...
//    bodyControlP1->setRightArmYawADCPosition(750);
    ...
    while (!stopped)
    {
        ...
        else
        {
            ...
            if (qDebugOn)
            {
                qDebug("Task#%lu: getRightArmYawPosition() (deg) = %f, target (deg) = %f, cutoffAngle = %f, dSigma = %f ? %f, direction: %s", getTaskId(), bodyControlP1->getRightArmYawPosition(), angle, cutoffAngle, abs(angle - bodyControlP1->getRightArmYawPosition()), sigma, (bodyControlP1->getRightArmYawMovementDirection() ? "down" : "up"));
            }
