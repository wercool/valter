  ...
  public:
    ...
    void executeCommand(string cmd)
    {
        ...
        if (cmd.find("setCentralCommandHostInfo") != std::string::npos)
        {
            ...
            platformControlP2->getTcpInterface()->sendCDRToCentralCommandHost(Valter::format_string("CDR~REMOTECD:%s:%s:%d:%s", platformControlP2->getControlDeviceId().c_str(), platformControlP2->getTcpInterface()->getLocalHostIP().c_str(), platformControlP2->getTcpInterface()->getPort(), platformControlP2->getControlDevice()->getStatus().c_str()));
