  ...
  public:
    ...
    void executeCommand(string cmd)
    {
        ...
        if (cmd.find("setCentralCommandHostInfo") != std::string::npos)
        {
            ...
            platformManipulatorAndIRBumper->getTcpInterface()->sendCDRToCentralCommandHost(Valter::format_string("CDR~REMOTECD:%s:%s:%d", platformManipulatorAndIRBumper->getControlDeviceId().c_str(), platformManipulatorAndIRBumper->getTcpInterface()->getCentralCommandHostIP().c_str(), platformManipulatorAndIRBumper->getTcpInterface()->getCentralCommandHostIPPort()));
