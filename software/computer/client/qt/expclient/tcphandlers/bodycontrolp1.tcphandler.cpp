  ...
  public:
    ...
    void executeCommand(string cmd)
    {
        ...
        if (cmd.find("setCentralCommandHostInfo") != std::string::npos)
        {
            ...
            bodyControlP1->getTcpInterface()->sendCDRToCentralCommandHost(Valter::format_string("CDR~REMOTECD:%s:%s:%d", bodyControlP1->getControlDeviceId().c_str(), bodyControlP1->getTcpInterface()->getCentralCommandHostIP().c_str(), bodyControlP1->getTcpInterface()->getCentralCommandHostIPPort()));
