  ...
  public:
    ...
    void executeCommand(string cmd)
    {
        ...
        if (cmd.find("setCentralCommandHostInfo") != std::string::npos)
        {
            ...
            bodyControlP1->getTcpInterface()->sendCDRToCentralCommandHost(Valter::format_string("CDR~REMOTECD:%s:%s:%d:%s", bodyControlP1->getControlDeviceId().c_str(), bodyControlP1->getTcpInterface()->getLocalHostIP().c_str(), bodyControlP1->getTcpInterface()->getPort(), bodyControlP1->getControlDevice()->getStatus().c_str()));
