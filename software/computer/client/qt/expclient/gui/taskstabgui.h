...
void tasksTabRefreshTimerUpdateWorker(Ui::MainWindow *ui)
{
    ...
    {
        std::map<int, string> rtmms = TaskManager::getInstance()->getRtmms();
        ...
        if (rtmms.size() > 0)
        {
            ...
            {
                int taskId = iterator->first;
                vector<string> taskDesc = Valter::split(((string)iterator->second), '~');
                ui->tasksTableWidget->setItem(i, 0, new QTableWidgetItem(Valter::format_string("%d", taskId).c_str()));//task id
