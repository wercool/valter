...
//
//	   entered when Timer0 RC compare interrupt asserts (50 msec period defined in pTC->TC_RC)
//
...
unsigned long Timer0Tick = 0;			// global variable counts interrupts


void Timer0IrqHandler (void) {

	unsigned int TimerIrq_dummy;
	volatile AT91PS_TC pTC = AT91C_BASE_TC0;// pointer to timer channel 0 register structure

	TimerIrq_dummy = pTC->TC_SR;		// read TC0 Status Register to clear it
	Timer0Tick++;				// increment the tick count
}
