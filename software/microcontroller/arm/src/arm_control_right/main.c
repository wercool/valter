...
int main(void)
{
    ...
    //thresholds
    unsigned int forearmLiftUpPositionThreshold = 70;
    unsigned int armLiftUpPositionThreshold = 765;
    //unsigned int limbLiftUpPositionReading = 0;
    ...
    //thresholds
    unsigned int handYawCWThreshold = 100;
    unsigned int handYawCCWThreshold = 700;
    unsigned int handPitchCWThreshold = 85;
    unsigned int handPitchCCWThreshold = 530;
    //positions
    ...
    //thresholds
    unsigned int forearmYawCWThreshold = 0;
    unsigned int forearmYawCCWThreshold = 1023;
    //positions
    unsigned int forearmYawPosition = 0;

    //globally watchers on/off
    unsigned char allWatchersOnOff = 1; //1 - on, 0 - off
...
//            pCDC.Write(&pCDC, (char *)msg, strlen((char *)msg));
            ...
            if (strcmp((char*) cmdParts, "STOPWATCHERS") == 0)
            {
                allWatchersOnOff = 0;
                continue;
            }
            if (strcmp((char*) cmdParts, "STARTWATCHERS") == 0)
            {
                allWatchersOnOff = 1;
                continue;
            }
        ...
        {
            unsigned int armLiftUpPositionReading_sum = 0;
            for (unsigned char i = 0; i < 5; i++)
            {
                armLiftUpPositionReading_sum += getValueChannel5();
                delay_us(100);
            }
            armLiftUpPositionReading = round((double)armLiftUpPositionReading_sum / (double) 5);
        }
        //check thresholds
        if (allWatchersOnOff == 1)
        {
            if (armLiftUpDirection == 0) //move down
            {
